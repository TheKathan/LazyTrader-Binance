using LazyTrader.Binance.Core.Enum;
...
public static class ValidationUtils
{
    ...
    public static void UserUniversalTransferRequestIsCorrect(string type, string? fromSymbol, string? toSymbol)
    {
        if (!Enum.TryParse(type.ToUpper(), out TransferType orderType)) throw new ArgumentValueIsIncorrectException(nameof(type));
        switch (orderType)
        {
            case TransferType.ISOLATEDMARGIN_MARGIN:
                ArgumentNotNull(fromSymbol, nameof(fromSymbol));
                break;
            case TransferType.MARGIN_ISOLATEDMARGIN:
                ArgumentNotNull(toSymbol, nameof(toSymbol));
                break;
            case TransferType.ISOLATEDMARGIN_ISOLATEDMARGIN:
                ArgumentNotNull(fromSymbol, nameof(fromSymbol));
                ArgumentNotNull(toSymbol, nameof(toSymbol));
                break;
            default:
