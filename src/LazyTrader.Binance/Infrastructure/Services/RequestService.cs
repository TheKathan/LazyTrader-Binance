...
internal class RequestService : IRequestService
{
	...
	public RequestService(ILogger<RequestService> logger, HttpClient httpClient, IConfiguration configuration)
	{
		...
		_httpClient.DefaultRequestHeaders.Add("recvWindow", configuration["Binance:RecvWindown"] ?? 10000.ToString());
	...
	public async Task<T> Get<T>(string uri, NameValueCollection? nameValueCollection, bool withArguments = true)
	{
		if (withArguments)
		{
	...
	public async Task<string> Post(string uri, NameValueCollection? nameValueCollection = null)
	{
		var queryString = QueryUtils.BuildQuery(_configuration["Binance:Secret"], nameValueCollection);
		...
		try
		{
			...
			ValidationUtils.SuccessfulRequest(response, result);
